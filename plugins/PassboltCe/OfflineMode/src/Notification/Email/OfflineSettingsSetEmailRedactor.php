...
class OfflineSettingsSetEmailRedactor implements SubscribedEmailRedactorInterface
{
    use LocatorAwareTrait;
    use SubscribedEmailRedactorTrait;

    public const TEMPLATE = 'Passbolt/OfflineMode.AD/settings_updated';
    ...
    {
        return [OfflineSettingsSetService::EVENT_SETTINGS_UPDATED];
    }
    ...
    /**
     * @param \Cake\Event\Event $event Settings-updated event.
    ...
    public function onSubscribedEvent(Event $event): EmailCollection
    {
        ...
        if (!$dto instanceof OfflineSettingsDto) {
            throw new InvalidArgumentException('`dto` is missing from event data.');
        }
        ...
        foreach ($admins as $recipient) {
            $emailCollection->addEmail(
                $this->createEmail($recipient, $operator, $dto, $uac->getUserIp(), $uac->getUserAgent()),
            );
        }
     ...
     * @param \Passbolt\OfflineMode\Model\Dto\OfflineSettingsDto $dto Settings DTO.
        ...
        return (new LocaleService())->translateString(
            ...
            function () use ($recipient, $operator) {
                return $operator->id === $recipient->id
                    ? __('You edited the Offline Mode settings')
                    : __('{0} edited the Offline Mode settings', $operator->profile->first_name);
            },
        );
    }
}
